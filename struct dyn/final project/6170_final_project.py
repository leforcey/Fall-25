...
class NodeFilter:
    ...
    def plot(self, u, ax=None, **kwargs):
        ...
        ax.tricontourf(tri_obj, u, cmap='rainbow')
...
class PlaneStress:
    def __init__(self, conn, X, bcs, forces={}, E=10.0, nu=0.3, rho=1.0, p=3.0, fltr=None, sigma_max=5.0, ks_param=30.0, eps=1e-4):
        self.conn = np.array(conn)
        self.X = np.array(X)
    ...
    def compute_ks_gradient(self,x):
        ...
        # (deriv of ks function wrt density variable)
...
# # --------------------------------------------------------
# # DISTRIBUTED LOAD ACROSS THE *TOP FACE*
# # --------------------------------------------------------

# P_total = 1.0            # total load you want to apply
# forces = {}

# # top face is j = ny-1
# top_nodes = nodes[:, -1]     # all i along top boundary

# load_per_node = P_total / len(top_nodes)

# for nd in top_nodes:
#     # Apply a downward distributed load (0 in x, -load_per_node in y)
#     forces[nd] = [0.0, -load_per_node]
